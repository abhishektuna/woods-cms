...
export function ProductPage() {
  ...
  const categoriesArr = Array.isArray(categoriesState.data)
    ? categoriesState.data
    : [];
  const subcategoriesArr = Array.isArray(subcategoriesState.data)
  ...
  const { categories, subcategories } = useMemo(() => {
    const cats = categoriesArr
      .map((c: any) => c.title || c.name)
      .filter(Boolean);
    const subcats = subcategoriesArr
    ...
    () => [
      {
        ...
        width: "15%",
        render: (p: any) => (
          <div className="font-medium text-gray-900 truncate" title={p.modelNo}>
            {p.modelNo}
          </div>
        ),
      },
      ...
      },
      {
        ...
        width: "30%",
        render: (p: any) => (
          <div className="line-clamp-2 text-gray-600" title={p.description}>
            {p.description || "-"}
          </div>
        ),
      },
      {
        ...
        render: (p: any) => (
          <div className="flex justify-center gap-1.5">
            <ActionButton action="view" onClick={() => handleView(p)} />
            <ActionButton action="edit" onClick={() => handleEdit(p)} />
            <ActionButton action="delete" onClick={() => handleDelete(p._id)} />
          </div>
        ),
      },
    ],
        ...
        <>
          ...
          {showViewModal && (
            <ViewModal
              ...
              title={`${viewingProduct?.modelNo || "Product"} Details`}
              size="lg"
            >
